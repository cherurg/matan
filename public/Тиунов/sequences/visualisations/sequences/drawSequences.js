function drawSequences (id) {
    ...
    function draw(allPoints, n, board, k, line)
    {
    	...
    	if (line)
    		...
    		str = "tiny";
    	var tmp = 0;  
		if (drawSequences.animated)   
		{
			var timer = setInterval(function()
			{
				for (i = tmp; i < Math.min(tmp + k, n); i++)
				{
					board.addPoint(allPoints[i][0], allPoints[i][1], {size: str});
					if (line)
			...
			for(tmp = 0; tmp < n; tmp++)    
			{
				board.addPoint(allPoints[tmp][0],allPoints[tmp][1], {size: str});
				if (line)
    ...
    function draw_segm(allPoints, n, board, time, ord)
    {
    	...
    	if (drawSequences.animated)
		{
			...
			for (i = tmp; i < Math.min(tmp + k, n); i++)
				board.addLine(allPoints[i], ord, allPoints[i], -ord, {width : 1, color : 0});
			tmp += k;
			if (tmp >= n)
				clearInterval(timer);
		}, time);
    		...
    		for (tmp = 0; tmp < n; tmp++)
    		{
    			board.addLine(allPoints[tmp], ord, allPoints[tmp], -ord, {width : 1, color : 0});
    		}
    }
	var a = {
		'0' : function () 
		{
			...
			var allPoints = [];
			var tmp = 1;
			if (drawSequences.vert_segm) 
				{
					var board=Plotter(id,{planeBorder: [0, 1, -1, 1], width: width, height: height});
					var n = 2000;
					for (var b = 2; b <= n; b++)
			...
			{
				if (drawSequences.log_scale)
				{
					var k = 100;
					var n = 40000;
					var board=Plotter(id,{planeBorder: [-0.05, Math.log(1001), 0, 1], width: width, height: height});
					for (var b = 2; b <= n; b++)
						for (var a = 1; a < b && tmp < n; a++)
				...
				else
				{
					var k = 5;
					var n = 2000;
					var board=Plotter(id,{planeBorder: [0, n, 0, 1], width: width, height: height});
					for (var b = 2; b <= n; b++)
						for (var a = 1; a < b && tmp < n; a++)
		...
		'1' : function ()
		{
			...
			if (drawSequences.vert_segm)
			{
				var board=Plotter(id,{planeBorder: [-2.1, 2.1, -1, 1], width: width, height: height});
				for (var i = 1; i <= n ; i++)
					{
			...
			else
			{
				if (drawSequences.log_scale)
				{
					var board=Plotter(id,{planeBorder: [0, Math.log(201), -5, 5], width: width, height: height});
					for (var i = 1; i <= n; i++)
				...
				{
					n = 1000;
					var board=Plotter(id,{planeBorder: [0, n, -5, 5], width: width, height: height});
					for (var i = 1; i <= n; i++)
		...
		'2' : function ()
		{
			...
			if (drawSequences.vert_segm)
			{
				var board=Plotter(id,{planeBorder: [-3, 3, -1, 1], width: width, height: height});
				for (var i = 1; i <= n ; i++)
		...
		'3' : function ()
		{
			...
			{
				var n = 10000;
				var board = Plotter(id,{planeBorder: [-1, 1, -1, 1], width: width, height: height});
				for (var i = 1; i <= n; i++)
			...
			else
			{
				if (drawSequences.log_scale)
				{
					var k = 30;
					var n = 20000;
					var board = Plotter(id,{planeBorder: [Math.log(10), Math.log(201), -1.5, 1.5], width: width, height: height});
					for (var i = 1; i <= n; i++)
				...
				else
				{
					var k = 40;
					var n = 20000;
					var board = Plotter(id,{planeBorder: [0, n, -1.5, 1.5], width: width, height: height});
					for (var i = 1; i <= n; i++)
		...
		'4' : function ()
		{
			...
			if (drawSequences.vert_segm)
			{
				...
				if (drawSequences.animated)
				{
					...
					var board = Plotter(id,{planeBorder: [1.9, 3, -1, 1], width: width, height: height});
					tmp = 1;
					timer = setInterval(function(){
				...
				{
					var n = 100;
					var board = Plotter(id,{planeBorder: [0, 4, -1, 1], width: width, height: height});
					for (var i = 1; i <= n; i++)
					{
						allPoints_1.push(Math.pow((1+1/i), i));
						allPoints_2.push(sum);
			...
			else
			{
				if (drawSequences.log_scale)
				{
					var k = 7;
					var n = 1000;
					var board = Plotter(id,{planeBorder: [0, Math.log(1000), 2, 3], width: width, height: height});
					for (var i = 1; i <= n; i++)
					{
				...
				else
				{
					var k = 1;
					var n = 100;
					var board = Plotter(id,{planeBorder: [-0.5, n, 0, 3], width: width, height: height});
					for (var i = 1; i <= n; i++)
					{
		...
		'5' : function ()
		{
			...
			{
				var n = 10000;
				var board = Plotter(id,{planeBorder: [-0.1, 0.6, -1, 1], width: width, height: height});
				while (m <= n)
					{
						for (var i = 1; i < j; i++)
						{
			...
			else
			{
				if (drawSequences.log_scale)
				{
					var n = 20000;
					var k = 40;
					var board = Plotter(id,{planeBorder: [0, Math.log(1001), 0, 0.55], width: width, height: height});
					while (m <= n)
					{
						for (var i = 1; i < j; i++)
				...
				else
				{
					var k = 4;
					var n = 2000;
					var board = Plotter(id,{planeBorder: [0, n, 0, 0.55], width: width, height: height});
					while (m <= n)
					{
						for (var i = 1; i < j; i++)
		...
		'7' : function ()
		{
			...
			{
				var n = 10000;
				var board=Plotter(id,{planeBorder: [-1, 50, -1, 1], width: width, height: height});
				while (k <= n)
					{
						allPoints.push(0);
						k++;
			...
			else
			{
				if (drawSequences.log_scale)
				{
					var m = 40;
					var n = 20000;
					var board = Plotter(id,{planeBorder: [-0.05, Math.log(1001), -20, 20], width: width, height: height});
					while (k <= n)
					{
				...
				else
				{
					var m = 4;
					var n = 2000;
					var board = Plotter(id,{planeBorder: [-20, n, -30, 30], width: width, height: height});
					while (k <= n)
					{
		...
		'8' : function ()
		{
			...
			{
				var n = 3000;
				var board = Plotter(id,{planeBorder: [-0.05, 1.6, -1, 1], width: width, height: height});
				for (var i = 1; i <= n; i++)
			...
			else
			{
				if (drawSequences.log_scale)
				{
					var k = 3;
					var n = 1000;
					var board = Plotter(id,{planeBorder: [-0.05, Math.log(1000), 0, 1.55], width: width, height: height});
					for (var i = 1; i <= n; i++)      
				...
				else
				{
					var k = 1;
					var n = 300;
					var board = Plotter(id,{planeBorder: [-1, n, 0, 1.2], width: width, height: height});
					for (var i = 1; i <= n; i++)      
		...
		'9' : function ()
		{
			...
			{
				var n = 3000;
				var board = Plotter(id,{planeBorder: [-0.05, 1.2, -1, 1], width: width, height: height});
				for (var i = 1; i <= n; i++)
			...
			else
			{
				if (drawSequences.log_scale)
				{
					var k = 3;
					var n = 1000;
					var board = Plotter(id,{planeBorder: [-0.05, Math.log(1000), 0, 1.5], width: width, height: height});
					for (var i = 1; i <= n; i++)      
				...
				else
				{
					var k = 1;
					var n = 300;
					var board = Plotter(id,{planeBorder: [-1, n, 0, 1.5], width: width, height: height});
					for (var i = 1; i <= n; i++)      
		...
		'11' : function ()
		{
			...
			{
				var n = 10000;
				var board = Plotter(id,{planeBorder: [0, 0.91, -1, 1], width: width, height: height});
				for (var i = 1; i <= n; i++)
			...
			else
			{
				...
				{
					var n = 1000;
					var board = Plotter(id,{planeBorder: [-0.005, Math.log(1000), 0, 1], width: width, height: height});
					for (var i = 1; i <= n; i++)
				...
				{
					var n = 500;
					var board = Plotter(id,{planeBorder: [-1, n, 0, 1], width: width, height: height});
					for (var i = 1; i <= n; i++)
		...
		'12' : function ()
		{
			...
			{
				var n = 1000;
				var board = Plotter(id,{planeBorder: [-1.1, 1.1, -1, 1], width: width, height: height});
				for (var i = 1; i <= n; i++)
			...
			else
			{
				...
				{
					var n = 300;
					var board = Plotter(id,{planeBorder: [0, Math.log(300), -1.5, 1], width: width, height: height});
					for (var i = 1; i <= n; i++)
				...
				{
					var n = 300;
					var board = Plotter(id,{planeBorder: [0, n, -1.5, 1], width: width, height: height});
					for (var i = 1; i <= n; i++)
		...
		'13' : function ()
		{
			...
			if (drawSequences.vert_segm)
			{
				var board = Plotter(id,{planeBorder: [-1, 2, -1, 1], width: width, height: height});
				for (var i = 1; i <= n; i++)
			...
			{
				var board = Plotter(id,{planeBorder: [-0.1, n, -1, 2], width: width, height: height});
				for (var i = 1; i <= n; i++)
		...
		'14' : function ()
		{
			...
			if (drawSequences.vert_segm)
			{
				var board = Plotter(id,{planeBorder: [-5, 5, -1, 1], width: width, height: height});
				for (var i = 1; i <= n; i++)
			...
			{
				var board = Plotter(id,{planeBorder: [-0.1, n, -5, 5], width: width, height: height});
				for (var i = 1; i <= n; i++)
		...
		'15' : function ()
		{
			...
			if (drawSequences.vert_segm)
			{
				var board = Plotter(id,{planeBorder: [-2.5, 2.5, -1, 1], width: width, height: height});
				for (var i = 1; i <= n; i++)
			...
			{
				var board = Plotter(id,{planeBorder: [-0.1, n, -3, 3], width: width, height: height});
				for (var i = 1; i <= n; i++)
...
drawSequences.updateItem = function (radioButton) {
	drawSequences.current=radioButton.value;
	(document.getElementById('Controls').getElementsByTagName('input')[1]).removeAttribute('disabled');
